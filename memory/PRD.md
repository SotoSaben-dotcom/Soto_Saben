...
- Halaman /pesan: dua mode — `?mode=reservasi` (khusus Cabang Berbah, min. 10 porsi, sound system, lokasi terkunci) dan `?mode=acara` (katering, min. 50 porsi, lokasi bebas, antar & racik di lokasi). Isi pesan WhatsApp berbeda per mode; pilihan menu multi-pilih dengan jumlah per item; estimasi harga real-time.
